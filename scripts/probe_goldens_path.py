#!/usr/bin/env python3
"""
P2 step 1: PROBE — can HF transformers load the NVFP4 checkpoint directly on
GPUs 4-7? Decides the goldens path (CONTEXT_AND_PLAN P2):

  PROBE PASSES -> goldens generate on GPUs 4-7 while vLLM stays up (idle);
                  no server downtime, reference precision = NVFP4 (same as
                  both engines -> ideal).
  PROBE FAILS  -> fallback: scheduled vLLM downtime, all-8-GPU bf16 reference
                  staged via /dev/shm (~2 TB model; slower, one-off).

This script only PROBES (load + one tiny greedy generation + timing). Golden
generation proper is generate_goldens.py, run after the path is chosen.

Probe run 1 (2026-07-24) finding: transformers 5.14.1 knows Inkling NATIVELY
(InklingConfig loaded; InklingTextConfig present in the CausalLM registry),
but the top-level config is a multimodal WRAPPER and is not registered under
AutoModelForCausalLM. Step 3 therefore tries a ladder of entry points and
records every failure, so a registry miss can never be mistaken for an
NVFP4-unsupported verdict again.

Run (vLLM may stay up — this is correctness work, not measurement; §4.3
forbids concurrent MEASUREMENT only):
  CUDA_VISIBLE_DEVICES=4,5,6,7 python scripts/probe_goldens_path.py
"""
import sys, time

MODEL_DIR = "/workspace/models/inkling-nvfp4"


def main():
    #1. version gate first: Inkling needs a recent transformers; fail informatively
    import transformers, torch
    print(f"[probe] transformers {transformers.__version__}, torch {torch.__version__}, "
          f"visible GPUs: {torch.cuda.device_count()}")
    if torch.cuda.device_count() != 4:
        sys.exit("[probe] expected exactly 4 visible GPUs — set CUDA_VISIBLE_DEVICES=4,5,6,7")

    from transformers import AutoConfig, AutoTokenizer

    #2. config + tokenizer load (cheap; catches class/availability issues)
    cfg = AutoConfig.from_pretrained(MODEL_DIR, trust_remote_code=True)
    print(f"[probe] config class: {type(cfg).__name__}")
    tok = AutoTokenizer.from_pretrained(MODEL_DIR, trust_remote_code=True)
    print(f"[probe] tokenizer ok, vocab {tok.vocab_size if hasattr(tok, 'vocab_size') else 'n/a'}")

    #3. resolve the right model class: InklingConfig is a multimodal wrapper,
    #   registered under image-text Auto classes, not CausalLM (probe run 1:
    #   transformers 5.14.1 knows Inkling natively — InklingTextConfig is in
    #   the CausalLM registry, the wrapper is not). Try doors in order; keep
    #   every failure so the verdict can distinguish registry vs quant errors.
    t0 = time.time()
    model, how, attempts = None, None, []

    def try_load(name, fn):
        try:
            return fn(), name
        except Exception as e:
            attempts.append(f"{name}: {type(e).__name__}: {str(e)[:200]}")
            return None, None

    if model is None:
        from transformers import AutoModelForImageTextToText
        model, how = try_load(
            "AutoModelForImageTextToText",
            lambda: AutoModelForImageTextToText.from_pretrained(
                MODEL_DIR, trust_remote_code=True,
                device_map="auto", torch_dtype="auto",
                max_memory={0: "250GiB", 1: "250GiB", 2: "250GiB", 3: "250GiB", "cpu": "1200GiB"}))
    if model is None:
        import transformers as tf
        cls = getattr(tf, "InklingForConditionalGeneration", None)
        if cls is not None:
            model, how = try_load(
                "InklingForConditionalGeneration (direct)",
                lambda: cls.from_pretrained(
                    MODEL_DIR, device_map="auto", torch_dtype="auto",
                    max_memory={0: "250GiB", 1: "250GiB", 2: "250GiB", 3: "250GiB",
                                "cpu": "1200GiB"}))
        else:
            attempts.append("InklingForConditionalGeneration: not exported by transformers")
    if model is None:
        from transformers import AutoModel
        model, how = try_load(
            "AutoModel",
            lambda: AutoModel.from_pretrained(
                MODEL_DIR, trust_remote_code=True,
                device_map="auto", torch_dtype="auto",
                    max_memory={0: "250GiB", 1: "250GiB", 2: "250GiB", 3: "250GiB",
                                "cpu": "1200GiB"}))
    if model is None:
        print(f"[probe] LOAD FAILED after {time.time()-t0:.0f}s; all attempts:")
        for a in attempts:
            print(f"  - {a}")
        print("[probe] VERDICT: read the errors above — class/registry errors "
              "mean fixable routing (paste them back; one more rung exists: "
              "loading the text tower directly); quant/dtype errors mean NVFP4 "
              "unsupported in transformers -> bf16 fallback path "
              "(all 8 GPUs, vLLM down).")
        sys.exit(2)
    print(f"[probe] load OK via {how} in {time.time()-t0:.0f}s")
    for i in range(4):
        print(f"[probe]   gpu{i}: {torch.cuda.memory_allocated(i)/2**30:.1f} GiB")

    #4. one tiny greedy generation — proves the forward path end to end.
    #   device_map="auto" shards the model; inputs go to the first device and
    #   accelerate routes activations across shards.
    prompt = "The capital of France is"
    ids = tok(prompt, return_tensors="pt").to("cuda:0")
    t0 = time.time()
    out = model.generate(**ids, max_new_tokens=8, do_sample=False)
    text = tok.decode(out[0][ids["input_ids"].shape[1]:])
    dt = time.time() - t0
    print(f"[probe] greedy 8 tokens in {dt:.1f}s ({8/dt:.2f} tok/s): {text!r}")

    #5. verdict + speed note for planning golden generation (50 prompts × 64 tok)
    est = 50 * 64 * dt / 8 / 60
    print(f"[probe] VERDICT: PASS — goldens can generate on GPUs 4-7 via {how} "
          f"with vLLM up.")
    print(f"[probe] speed estimate for the parity set: ~{est:.0f} min "
          f"(fine if <120; else consider fewer max_new tokens)")
    #6. sanity: reference should agree with vLLM on this trivial prompt
    if "Paris" not in text:
        print("[probe] WARNING: expected 'Paris' in output — investigate before "
              "trusting this path for goldens")


if __name__ == "__main__":
    main()