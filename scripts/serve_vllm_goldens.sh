...
export VLLM_BATCH_INVARIANT=1   # deterministic kernels for GOLDENS ROLE ONLY;
                                # benchmark server never runs with this (perf cost;
                                # canonical contract measures vLLM at its fastest)

mkdir -p /workspace/logs
LOG="/workspace/logs/vllm_goldens_$(date +%Y%m%d_%H%M%S).log"
