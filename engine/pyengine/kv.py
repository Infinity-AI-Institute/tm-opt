...
class PagedKV:
    ...
    def __init__(self, num_pages, page_size, kv_heads, head_dim, device,
                 dtype=torch.bfloat16, free_order=None):
        ...
        self.free = list(free_order) if free_order is not None else list(
            range(num_pages))
        assert sorted(self.free) == list(range(num_pages)), "bad free order"
    ...
    def append(self, k, v):
        ...
        last_page = (self.n + T - 1) // self.page_size
...
class LayerKV:
    ...
    def __init__(self, mc, layer_idx, device, num_pages=None,
                 free_order=None, dtype=torch.bfloat16):
        ...
        if self.is_global:
            assert num_pages is not None, "global layer needs a page budget"
            self.cache = PagedKV(num_pages, PAGE_SIZE, kv_heads, mc.head_dim,
                                 device, dtype, free_order)
        else:
