"""exp-0025 evidence: the fused prefill sconv measured IN SITU.
...
traversal, with PYENGINE_FUSED_SCONV toggled between arms.
...
is ~800-1024 (the same LENS t_pipeline_prefill/t_decode_first used).
  ...
  arm b  TOKENS — the same schedule under both arms. Here equality is a
         REAL claim, not a sanity signal: the kernel keeps the reference's
         tap order and fp32 arithmetic, so t_fused_sconv arm a gates it
         bitwise per call site and the whole schedule must agree token for
         token. A mismatch would mean the isolated arm missed a shape.
...
def _run(eng, dev0, fused, warm=False):
    ...
    #1. the arm switch is the module global sconv_prefill reads per call
    #   (prefill is not captured, so toggling between runs is safe)
    pmodel._FUSED_SCONV = fused
...
def main():
    ...
    print(f"[t0025i] resident in {time.time() - t0:.0f} s, layer split "
          f"{splits}", flush=True)
    #2. warm BOTH arms' JIT (the fused kernel compiles one specialization
    #   per (dtype, K) pair; the eager arm warms cuDNN's depthwise conv
    #   and the fp32 temporaries' allocator) — discarded
    t0 = time.time()
    _run(eng, dev0, True, warm=True)
    _run(eng, dev0, False, warm=True)
    print(f"[t0025i] warm-up {time.time() - t0:.0f} s", flush=True)
    #3. interleaved arms
    ...
    print(f"[t0025i] arm a  SCHEDULE total: {tot_off:.3f} -> {tot_on:.3f} "
    ...
    print(f"[t0025i] arm b  eager tokens == fused tokens: {same} "
          f"({len(tok_off1)} seqs, {ntok} tokens)", flush=True)
    if not same:
        diff = [k for k in tok_off1 if tok_off1[k] != tok_on1.get(k)]
        print(f"[t0025i]        differing seqs: {diff}", flush=True)
        for k in diff[:2]:
            print(f"[t0025i]        {k}: eager {tok_off1[k]} vs fused "
                  f"{tok_on1[k]}", flush=True)
    #5. arm c — same-schedule determinism of the fused arm (the gate)
    det = tok_on1 == tok_on2
    print(f"[t0025i] arm c  fused run1 == run2 (bitwise tokens): {det}",
          flush=True)
    #   eager repeatability, for reference on the same schedule
    print(f"[t0025i] arm c  eager run1 == run2: {tok_off1 == tok_off2}",
          flush=True)
    ok = det
    print(f"[t0025i] RESULT {'PASS' if ok else 'FAIL'}", flush=True)
